...
\begin{itemize}
\item PRS-intro001-overview: a topic-stratified PRS reference list
\item PRS-intro101-foundation: GWAS, $h^2$ and prediction
\item {\bf PRS-intro201-basic}: PRS calculation and performance evaluation
\item {\bf PRS-intro301-basic-plus}: some obvious or not so obvious follow-up Qs
\item {\bf PRS-intro401-heterogeneity}: heterogeneity and transportabiliy
\item PRS-intro501-LD: linkage disequilibrium
\end{itemize}
...
\item Step-by-step {\bf R-assisted `hand calculation'} with (almost) {\bf no black box} 
...
\begin{itemize}
\item Workshop teaching: $\approx$2 hours, {\bf covering only some of the notes.} {\color{red}{\bf Please go over the 001 and 101 slides distributed prior to the workshop carefully, so that more workshop time can be dedicated to other materials. Thank you!}}
...
\footnotesize [Prof. Linbo Wang, UofT Stat](https://sites.google.com/site/linbowangpku/home) 
...
- effects of sample size n and genetic effect beta on AUC: easy to answer.  
- answers to these Qs are less obvious. If we decrease beta from 0.3 to 0.1 but increase the number of truly associated SNPs from 10 to 90:   
h2 and SNP h2? AUC in general? AUC between PRS.gw and PRS.01?  
